...
function mention(input,user){var regex = new RegExp(`@\s*(.*?)\s* `, "g"),matches = [], matchesResult = {}, m;while (m = regex.exec(input)) {matches.push(m[0]);};matches.forEach((e)=>{var e1 = e,e2 = e;e2 = e2.replace(" ",`</span>`).replace("@",`<span class="simple-tag simple-small simple-theme-d3" onclick="location.replace('/u/${e1.replace("@","").replace(" ","")}');">@`);matchesResult[e1] = e2;});Object.keys(matchesResult).forEach((e)=>{input=input.replace(e,matchesResult[e]);});return input;}
